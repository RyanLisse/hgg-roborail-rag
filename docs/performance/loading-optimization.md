...
const VectorStoreMonitoring = lazy(
  () => import("@/components/vector-store-monitoring"),
);
```
...
export function useLoadingState() {
  ...
  const loadingProgress =
    Object.values(loadingStates).filter((state) => !state).length / 4;
