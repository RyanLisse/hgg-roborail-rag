...
class DatabaseValidator {
  ...
  async validateConnection(name, url, options = {}) {
    ...
    try {
      ...
      const result =
        await sql`SELECT 1 as test, version(), current_database(), current_user, inet_server_addr(), inet_server_port()`;
      ...
      logInfo(
        `Server: ${result[0].inet_server_addr}:${result[0].inet_server_port}`,
      );
  ...
  async testConnectionPooling() {
    ...
    try {
      ...
      this.testResults.pooling = {
        ...
        performance_difference: Math.round(
          Math.abs(pooledDuration - directDuration),
        ),
      };

      logSuccess(
        `Pooled: ${Math.round(pooledDuration)}ms, Direct: ${Math.round(directDuration)}ms`,
      );
    } catch (error) {
      this.testResults.pooling = {
        success: false,
  ...
  async testBasicOperations(url) {
    ...
    try {
      ...
      const selectResult =
        await sql`SELECT NOW() as current_time, 'test' as message`;
  ...
  async validateSchema(url) {
    ...
    try {
      ...
      logInfo(
        `Permissions - Connect: ${perm.can_connect}, Create: ${perm.can_create}, Temp: ${perm.can_temp}`,
      );
      ...
      logSuccess(
        `Accessible schemas: ${schemas.map((s) => s.schema_name).join(', ')}`,
      );
  ...
  async performanceTest(url) {
    ...
    try {
      ...
      logSuccess(
        `10 concurrent queries completed in ${Math.round(concurrentDuration)}ms`,
      );
      logInfo(
        `Unique process IDs: ${new Set(concurrentResults.map((r) => r[0].process_id)).size}`,
      );
  ...
  generateReport() {
    ...
    const passedTests = Object.values(this.testResults).filter(
      (test) =>
        test.success ||
        (typeof test === 'object' &&
          Object.values(test).some((v) => v && v.success !== false)),
    ).length;

    if (passedTests === totalTests) {
      logSuccess(`All ${totalTests} test categories passed!`);
      logSuccess(
        'Database connection is fully validated and ready for production use.',
      );
    } else {
    ...
    console.log(
      `${colors.green}Main DB:${colors.reset} ${DB_CONFIG.MAIN_URL.replace(/:[^:@]*@/, ':***@')}`,
    );
    console.log(
      `${colors.green}Test DB:${colors.reset} ${DB_CONFIG.TEST_URL?.replace(/:[^:@]*@/, ':***@') || 'Not configured'}`,
    );
