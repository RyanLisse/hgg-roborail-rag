...
test.describe('Vector Store Integration', () => {
  ...
  test('should show database selector in main chat interface', async ({ page }) => {
    ...
    const selectorButton = page.locator('[data-testid="database-selector"], .database-selector, button:has-text("Database"), button:has-text("Data Sources")').first();
    await expect(selectorButton).toBeVisible();
  });

  test('should show vector store sources in database selector', async ({ page }) => {
    // Should be on main chat already
    await page.waitForSelector('[data-testid="multimodal-input"]', { timeout: 15000 });
    
    // Look for database/source selection interface
    const sourceSelector = page.locator('[data-testid="database-selector"], button:has-text("Data Sources")').first();
    ...
    if (await sourceSelector.isVisible()) {
      ...
      const vectorStoreOptions = page.locator('[role="menuitem"], [role="option"]');
  ...
  test('should show file upload area in main chat', async ({ page }) => {
    ...
    const uploadArea = page.locator('[data-testid="attachments-button"], input[type="file"], button:has-text("attach")');
