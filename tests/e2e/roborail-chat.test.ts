...
test.describe('RoboRail Chat with Vector Store', () => {
  ...
  test('Database selector shows OpenAI vector store as default', async () => {
    ...
    const databaseSelector = chatPage.page.locator('[data-testid="database-selector"]');
  ...
  test('Send RoboRail calibration question and get relevant response', async () => {
    ...
    expect(assistantMessage.content.toLowerCase()).toMatch(/(calibrat|roborail|measurement|accuracy)/);
  ...
  test('Send RoboRail safety question and get documentation-based response', async () => {
    ...
    await chatPage.sendUserMessage('What are the safety procedures for RoboRail operations?');
    await chatPage.isGenerationComplete();

    const assistantMessage = await chatPage.getRecentAssistantMessage();
    
    // Check for safety-related content
    expect(assistantMessage.content.toLowerCase()).toMatch(/(safety|ppe|procedure|protective|equipment)/);
    expect(assistantMessage.content.length).toBeGreaterThan(100);
  });
  ...
  test('Send RoboRail accuracy question and get technical specifications', async () => {
    ...
    await chatPage.sendUserMessage('What is the measurement accuracy of RoboRail?');
    await chatPage.isGenerationComplete();

    const assistantMessage = await chatPage.getRecentAssistantMessage();
    
    // Check for accuracy/precision information
    expect(assistantMessage.content.toLowerCase()).toMatch(/(accuracy|precision|mm|degree|measurement|0\.1)/);
  ...
  test('RoboRail suggested questions work correctly', async () => {
    ...
    const suggestions = chatPage.page.locator('[data-testid="suggested-actions"] button');
    await expect(suggestions.first()).toBeVisible();
    
    // Click on a RoboRail-specific suggestion
    const roborailSuggestion = suggestions.filter({ hasText: /roborail|calibrat|measurement/i });
  ...
  test('File search tool is used for RoboRail queries', async () => {
    ...
    await chatPage.sendUserMessage('Tell me about RoboRail troubleshooting procedures');
    await chatPage.isGenerationComplete();
    ...
    expect(assistantMessage.content.length).toBeGreaterThan(100);
    expect(assistantMessage.content.toLowerCase()).toMatch(/(roborail|troubleshoot|procedure|issue|problem)/);
  });
  ...
  test('Chat flow works with model selector and vector store', async () => {
    ...
    const modelSelector = chatPage.page.locator('[data-testid="model-selector"]');
    ...
    const assistantMessage = await chatPage.getRecentAssistantMessage();
    expect(assistantMessage.content.toLowerCase()).toMatch(/(roborail|operat|system|procedure)/);
    expect(assistantMessage.content.length).toBeGreaterThan(100);
  });
  ...
  test('Vector store responses include file references', async () => {
    ...
    const hasFileReference = response.includes('manual') || 
