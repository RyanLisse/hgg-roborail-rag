...
export class StagehandChatPage {
    ...
    try {
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error('Stagehand init timeout')), 30000),
        ),
      ]);
  ...
  async isGenerationComplete() {
    try {
      ...
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error('UI completion timeout')), 15000),
        ),
      ]);
    } catch (error) {
  ...
  async waitForResponseUsingAI(timeout = 45000): Promise<boolean> {
    try {
      await this.isGenerationComplete();
      return true;
    } catch (error) {
  ...
  async verifyWeatherResponseUsingAI(): Promise<boolean> {
    ...
    return weatherResponse?.hasWeatherInfo || false;
  }
