...
async function testVectorStoreMonitoring() {
  ...
  try {
    ...
    const { getVectorStoreMonitoringService } = await import(
      './lib/vectorstore/monitoring.js'
    );
    const { getOpenAIVectorStoreService } = await import(
      './lib/vectorstore/openai.js'
    );
    const { getNeonVectorStoreService } = await import(
      './lib/vectorstore/neon.js'
    );
    const { getUnifiedVectorStoreService } = await import(
      './lib/vectorstore/unified.js'
    );
    const { initializeVectorStoreMonitoring } = await import(
      './lib/vectorstore/monitoring-init.js'
    );
    ...
    console.log(
      `   - OpenAI: ${openaiService.isEnabled ? 'Enabled' : 'Disabled'}`,
    );
    ...
    if (openaiService.isEnabled) {
      ...
      console.log(
        `   OpenAI Health: ${openaiHealth.isHealthy ? '✅' : '❌'} ${openaiHealth.error || ''}`,
      );
    }
    ...
    const testScenarios = [
      ...
      },
      {
        ...
        latency: 2000,
        success: true,
        operation: 'cross_provider_search',
      },
      {
        provider: 'openai',
        latency: 5000,
        success: false,
        operation: 'timeout_search',
      },
      {
        provider: 'neon',
        latency: 600,
    ...
    monitoring.recordTokenUsage('openai', 1500, {
      model: 'gpt-4o-mini',
      test: true,
    });
    monitoring.recordTokenUsage('openai', 800, {
    ...
    for (const provider of providers) {
      ...
      console.log(
        `   - Avg Latency: ${Math.round(performance.averageLatency)}ms`,
      );
      console.log(
    ...
    console.log(
      `   - Overview: ${Object.keys(dashboardData.overview).length} providers`,
    );
    console.log(
      `   - Health Status: ${dashboardData.healthStatus.length} services`,
    );
    console.log(
      `   - Recent Errors: ${dashboardData.recentErrors.length} errors`,
    );
    ...
    if (openaiService.isEnabled) {
      ...
      try {
        ...
        if (openaiService.defaultVectorStoreId) {
          ...
          console.log(
            `   ✅ Search completed: ${searchResult.success ? 'Success' : 'Failed'}`,
          );
          console.log(
            `   📊 Results: ${searchResult.results.length} found in ${searchResult.executionTime}ms`,
          );
        }
      } catch (error) {
    ...
    if (neonService.isEnabled) {
      ...
      try {
        ...
        const embedding = await neonService.generateEmbedding(
    ...
    console.log(
      '🎉 Vector Store Monitoring Integration Test Completed Successfully!',
    );
    console.log('');
    ...
    console.log(
      '   4. Monitor real vector store operations in your application',
    );
  } catch (error) {
    ...
    console.error(
      '   4. Review the monitoring documentation in VECTOR_STORE_MONITORING.md',
    );
    process.exit(1);
  }
}
