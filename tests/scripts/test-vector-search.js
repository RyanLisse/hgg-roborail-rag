  ...
  try {
    ...
    const assistant = await fetch('https://api.openai.com/v1/assistants', {
      ...
      body: JSON.stringify({
        ...
        instructions:
          'You help search through RoboRail documentation. Be concise in your responses.',
        model: 'gpt-4o-mini',
        tools: [
          {
            type: 'file_search',
    ...
    for (const query of testQueries) {
      ...
      const run = await fetch(
        `https://api.openai.com/v1/threads/${threadData.id}/runs`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${API_KEY}`,
      ...
      if (runStatus.status === 'completed') {
        ...
        if (lastMessage.role === 'assistant') {
          console.log(
            `   ✅ Response: ${lastMessage.content[0].text.value.substring(0, 200)}...`,
          );
          ...
          if (annotations.length > 0) {
            console.log(
              `   📄 Sources found: ${annotations.length} file references`,
            );
          }
        }
      } else {
