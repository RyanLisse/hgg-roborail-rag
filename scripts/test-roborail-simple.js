  ...
  for (let i = 0; i < TEST_QUESTIONS.length; i++) {
    ...
    try {
      ...
      const assistant = await openai.beta.assistants.create({
        ...
        instructions: 'You are a helpful assistant that answers questions about RoboRail based on the provided documentation. Be specific and cite relevant information.',
        model: 'gpt-4o-mini',
      ...
      if (runStatus.status === 'completed') {
        ...
        if (response.role === 'assistant' && response.content[0].type === 'text') {
          ...
          console.log(`   📝 Answer: ${answerText.substring(0, 150)}${answerText.length > 150 ? '...' : ''}`);
          console.log(`   📚 Citations: ${citations.length} found`);
          
          // Check if response contains RoboRail-specific content
          const roboRailKeywords = ['roborail', 'measurement', 'calibration', 'accuracy', 'safety'];
