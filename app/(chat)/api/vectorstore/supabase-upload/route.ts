...
export async function POST(request: NextRequest): Promise<NextResponse<UploadResponse | ErrorResponse>> {
  ...
  try {
    ...
    if (file && !rawContent) {
      // Validate file type
      const isValidType = SUPPORTED_TEXT_TYPES.some(type => file.type === type) ||
    ...
    try {
      ...
      return NextResponse.json(
        { 
          ...
          details: error instanceof z.ZodError ? error.errors[0].message : "Validation failed",
          timestamp: new Date().toISOString() 
        }, 
        { status: 400 }
      );
    ...
    // Get updated stats to verify upload
    ...
    return NextResponse.json({
      ...
      documentId,
      title: documentTitle,
      message: "Document uploaded and indexed successfully with Cohere embed-v4.0",
      stats: {
...
export async function GET(): Promise<NextResponse<StatsResponse | ErrorResponse>> {
  try {
    ...
    console.error("Detailed stats error:", errorDetails);
