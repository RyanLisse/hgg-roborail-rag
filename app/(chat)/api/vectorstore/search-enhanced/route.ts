...
import { getUnifiedVectorStoreService, UnifiedSearchRequest, type EnhancedSearchResponse } from '@/lib/vectorstore/unified';
import { z } from 'zod';

// Enhanced search request schema
const EnhancedSearchRequestSchema = z.object({
  query: z.string().min(1),
  sources: z.array(z.enum(['openai', 'neon', 'memory', 'unified'])).default(['openai', 'memory']),
  ...
  // Performance options
  timeout: z.number().min(1000).max(30000).default(10000), // 10 second default timeout
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🎯 Features enabled: scoring=${validatedRequest.enableRelevanceScoring}, cross-encoder=${validatedRequest.enableCrossEncoder}, diversification=${validatedRequest.enableDiversification}`);
    
    if (validatedRequest.queryContext) {
      console.log(`📊 Query context: ${validatedRequest.queryContext.type || 'auto-detect'} (${validatedRequest.queryContext.domain || 'general'}, ${validatedRequest.queryContext.complexity || 'auto'})`);
    }
    ...
    const timeoutPromise = new Promise((_, reject) => 
      setTimeout(() => reject(new Error('Search timeout')), validatedRequest.timeout)
    );

    const result = await Promise.race([searchPromise, timeoutPromise]) as EnhancedSearchResponse;
    ...
    if (error instanceof Error && error.message.includes('timeout')) {
      errorResponse.message = 'Search timed out. Try reducing maxResults or increasing timeout.';
    }
  ...
  try {
    ...
    const response = {
      ...
      documentation: {
        ...
        method: 'POST',
        description: 'Enhanced vector store search with advanced relevance scoring',
        parameters: {
          query: 'Required: Search query string',
          enableRelevanceScoring: 'Optional: Enable multi-factor relevance scoring (default: true)',
          enableCrossEncoder: 'Optional: Enable cross-encoder reranking (default: false)',
          enableDiversification: 'Optional: Enable result diversification (default: true)',
