...
describe('Errors', () => {
  ...
  describe('ChatSDKError', () => {
    it('should create error with correct properties', () => {
      ...
      expect(error.message).toBe('You have exceeded your maximum number of messages for the day. Please try again later.');
    });
    ...
    it('should support retry configuration', () => {
      const error = new ChatSDKError('network:vectorstore', 'Network timeout', true, 5000);
    ...
    describe('fromVectorStoreError', () => {
      ...
      it('should support custom surface', () => {
        ...
        const chatError = ChatSDKError.fromVectorStoreError(vectorStoreError, 'agent');
  ...
  describe('getMessageByErrorCode', () => {
    ...
    it('should return specific messages for vectorstore errors', () => {
      expect(getMessageByErrorCode('network:vectorstore')).toBe('Network connection to vector store failed. Please check your internet connection and try again.');
      expect(getMessageByErrorCode('authentication:vectorstore')).toBe('Vector store authentication failed. Please check your API configuration.');
      expect(getMessageByErrorCode('rate_limit:vectorstore')).toBe('Vector store rate limit exceeded. Please wait and try again.');
    });

    it('should return specific messages for agent errors', () => {
      expect(getMessageByErrorCode('network:agent')).toBe('Agent service network error. Please check your connection and try again.');
      expect(getMessageByErrorCode('authentication:agent')).toBe('Agent service authentication failed. Please check your configuration.');
    });
  ...
  describe('visibilityBySurface', () => {
    it('should define visibility for all surfaces', () => {
      const surfaces: Surface[] = [
        'chat', 'auth', 'api', 'stream', 'database', 
        'history', 'vote', 'document', 'suggestions', 
        'vectorstore', 'agent'
      ];
    ...
    it('should set user-facing surfaces to response visibility', () => {
      const userFacingSurfaces: Surface[] = [
        'chat', 'auth', 'api', 'stream', 'history', 
        'vote', 'document', 'suggestions', 'agent'
      ];
