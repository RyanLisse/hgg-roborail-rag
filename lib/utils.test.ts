...
describe('Utils', () => {
  ...
  describe('fetcher', () => {
    ...
    it('should throw ChatSDKError on HTTP error', async () => {
      const mockErrorData = { code: 'auth:invalid_api_key', cause: 'Invalid key' };
      const mockResponse = {
        ok: false,
  ...
  describe('fetchWithErrorHandlers', () => {
    ...
    it('should throw ChatSDKError on HTTP error', async () => {
      const mockErrorData = { code: 'auth:rate_limit', cause: 'Too many requests' };
      const mockResponse = {
        ok: false,
        json: vi.fn().mockResolvedValue(mockErrorData),
      };
      ...
      await expect(fetchWithErrorHandlers('/test-url')).rejects.toThrow(ChatSDKError);
    });
    ...
    it('should pass through other errors when online', async () => {
      ...
      await expect(fetchWithErrorHandlers('/test-url')).rejects.toThrow(customError);
    });
  });
