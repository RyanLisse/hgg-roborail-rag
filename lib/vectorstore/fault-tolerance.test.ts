...
describe("CircuitBreaker", () => {
  ...
  beforeEach(() => {
    ...
    circuitBreaker = new CircuitBreaker("test", {
      ...
      monitorWindowMs: 10_000, // Changed from 5000 to meet minimum
      minimumThroughput: 2,
      successThreshold: 2,
    });
  });
...
describe("FallbackManager", { timeout: 10_000 }, () => {
  ...
  test("should handle provider timeouts", { timeout: 20_000 }, async () => {
...
describe("FaultTolerantService", { timeout: 10_000 }, () => {
  ...
  beforeEach(() => {
    ...
    faultTolerantService = new FaultTolerantService<string>("test-service", {
      ...
      healthCheckIntervalMs: 60_000, // Set to a high value to avoid frequent health checks in tests
      retryConfig: {
        maxRetries: 2,
      ...
      circuitBreakerConfig: {
        ...
        monitorWindowMs: 10_000, // Added to meet minimum requirement
      },
    });
  });
  ...
  test(
    "should execute successful operations",
    { timeout: 20_000 },
    async () => {
  ...
  test("should collect metrics", { timeout: 20_000 }, async () => {
    const successOperation = vi.fn().mockResolvedValue("success");
...
describe("FaultToleranceFactory", { timeout: 10_000 }, () => {
  beforeEach(() => {
    vi.useFakeTimers({ shouldAdvanceTime: true });
  });
