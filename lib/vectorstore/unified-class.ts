...
export class UnifiedVectorStore extends BaseVectorStoreService {
  ...
  async upload(documents: any[]) {
    const service = await this.service;
    return this.withRetry(() => Promise.all(documents.map(doc => service.addDocument(doc))), 'upload');
  }

  async delete(ids: string[]) {
    const service = await this.service;
    return this.withRetry(() => Promise.all(ids.map(id => service.deleteDocument(id, 'unified'))), 'delete');
  }
  ...
  async healthCheck() {
    ...
    return this.withRetry(async () => {
      const sources = await service.getAvailableSources();
      return {
        message: sources.length > 0 ? 'Unified service is healthy' : 'No sources available',
  ...
  async getStats() {
    ...
    return this.withRetry(async () => {
      ...
      return {
        documentsCount: Object.values(sourceStats).reduce((sum, stat) => sum + (stat.count || 0), 0),
