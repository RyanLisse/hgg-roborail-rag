...
export async function initializeVectorStoreMonitoring(): Promise<void> {
  ...
  try {
    ...
    if (neonService.isEnabled) {
      ...
      console.log('⚠️ Neon vector store monitoring disabled (no connection string)');
    }
      ...
      try {
        ...
        switch (provider) {
          ...
          case 'neon':
            if (neonService.isEnabled) {
              ...
              try {
                ...
                result = { isHealthy: true, vectorStoreStatus: 'Database connection active' };
              } catch (error) {
                result = { 
                  isHealthy: false, 
          ...
          case 'unified': {
            ...
            const hasAvailableServices = openaiService.isEnabled || neonService.isEnabled;
            result = { 
              ...
              error: hasAvailableServices ? undefined : 'No underlying services enabled',
            };
            break;
          }
    ...
    console.log(`🩺 Starting health check scheduler for providers: ${providersToMonitor.join(', ')}`);
    healthCheckCleanup = startHealthCheckScheduler(monitoringService, providersToMonitor);
    ...
    initialHealthChecks.forEach((result, index) => {
      ...
      if (result.status === 'fulfilled') {
        console.log(`✅ Initial health check for ${provider}: ${result.value.isHealthy ? 'Healthy' : 'Unhealthy'}`);
      } else {
        console.error(`❌ Initial health check failed for ${provider}:`, result.reason);
      }
    });
