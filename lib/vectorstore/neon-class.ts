...
export class NeonVectorStore extends BaseVectorStoreService {
  ...
  async delete(ids: string[]) {
    const service = await this.service;
    return this.withRetry(() => Promise.all(ids.map(id => service.deleteDocument(id))), 'delete');
  }

  async healthCheck() {
    const service = await this.service;
    return this.withRetry(async () => ({
      message: service.isEnabled ? 'Neon service is healthy' : 'Neon service is disabled',
