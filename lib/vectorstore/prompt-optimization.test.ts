...
describe('PromptOptimizationEngine', () => {
  describe('classifyQuery', () => {
    it('should classify technical queries correctly', async () => {
      const query = 'How do I use the RoboRail API endpoint for authentication?';
    ...
    it('should classify troubleshooting queries correctly', async () => {
      const query = 'RoboRail automation workflow not working, getting error 500';
      const context: QueryContext = { type: undefined };
      const result = await PromptOptimizationEngine.optimizeQuery(query, context);
    ...
    it('should classify configuration queries correctly', async () => {
      const query = 'How to configure RoboRail environment variables and settings?';
      const context: QueryContext = { type: undefined };
      const result = await PromptOptimizationEngine.optimizeQuery(query, context);
  ...
  describe('determineComplexity', () => {
    ...
    it('should classify intermediate queries', async () => {
      const query = 'How to implement RoboRail webhook authentication with OAuth2?';
    ...
    it('should classify advanced queries', async () => {
      const query = 'How to implement custom RoboRail microservices architecture with containerization, load balancing, and advanced monitoring observability?';
      const context: QueryContext = { type: 'technical' };
      const result = await PromptOptimizationEngine.optimizeQuery(query, context);
  ...
  describe('domain-specific optimization', () => {
    ...
    it('should include domain-specific keywords in expansion', async () => {
      ...
      const hasIntegrationKeywords = result.expandedQueries.some(q => 
  ...
  describe('conversation context optimization', () => {
    it('should incorporate conversation history', async () => {
      ...
      const context: QueryContext = {
        ...
        conversationHistory: [
          { role: 'user', content: 'I am having issues with RoboRail automation', timestamp: Date.now() - 60000 },
          { role: 'assistant', content: 'What specific automation features are you having trouble with?', timestamp: Date.now() - 30000 },
        ],
      };
      const result = await PromptOptimizationEngine.optimizeQuery(query, context);
      
      expect(result.contextualPrompt).toContain('automation');
      expect(result.expandedQueries.some(q => q.includes('automation'))).toBe(true);
    });
    ...
    it('should include previous queries context', async () => {
      ...
      expect(result.expandedQueries.some(q => q.includes('webhook setup'))).toBe(true);
    });
  });
  ...
  describe('relevance estimation', () => {
    it('should estimate higher relevance for specific queries', async () => {
      const query = 'RoboRail automation API webhook configuration with OAuth2 authentication';
      const context: QueryContext = { 
        type: 'api',
        domain: 'automation',
...
describe('QueryExpansionEngine', () => {
  describe('expandWithSynonyms', () => {
    ...
    it('should add related terms', () => {
      ...
      expect(result.some(q => q.includes('webhook') || q.includes('endpoint'))).toBe(true);
    });
  });
  ...
  describe('generateDomainVariations', () => {
    it('should generate RoboRail-specific variations', () => {
      ...
      const result = QueryExpansionEngine.generateDomainVariations(query, 'automation');
    ...
    it('should include domain keywords', () => {
      ...
      const result = QueryExpansionEngine.generateDomainVariations(query, 'configuration');
      
      const domainConfig = ROBORAIL_DOMAIN_PROMPTS.configuration;
      expect(result.some(q => 
  ...
  describe('generateContextualVariations', () => {
    it('should generate variations based on conversation history', () => {
      ...
      const context: QueryContext = {
        conversationHistory: [
          { role: 'user', content: 'RoboRail webhook not working', timestamp: Date.now() },
        ],
      };
      
      const result = QueryExpansionEngine.generateContextualVariations(query, context);
    ...
    it('should build on previous queries', () => {
      ...
      expect(result.some(q => q.includes('authentication configuration'))).toBe(true);
    });
  });
});
...
describe('ContextWindowManager', () => {
  describe('optimizeDocumentContext', () => {
    it('should optimize large document sets', () => {
      const documents = Array.from({ length: 10 }, (_, i) => ({
        content: `Document ${i} content about RoboRail automation workflows and configuration settings. This document contains detailed information about ${i % 3 === 0 ? 'automation' : i % 3 === 1 ? 'configuration' : 'integration'} features.`.repeat(50),
        metadata: { index: i },
      }));
      
      const query = 'automation workflows';
      const optimized = ContextWindowManager.optimizeDocumentContext(documents, query, 4000);
    ...
    it('should summarize documents when context limit is exceeded', () => {
      const documents = [{
        content: 'Very long document content about RoboRail automation. '.repeat(1000),
        metadata: { type: 'automation' },
      }];
      
      const query = 'automation';
      const optimized = ContextWindowManager.optimizeDocumentContext(documents, query, 500);
      
      expect(optimized.length).toBeGreaterThan(0);
      if (optimized[0].metadata?.summarized) {
        expect(optimized[0].content.length).toBeLessThan(documents[0].content.length);
      }
    });
  });
  ...
  describe('optimizeConversationContext', () => {
    it('should limit conversation history by token count', () => {
      ...
      const optimized = ContextWindowManager.optimizeConversationContext(history, 'current query', 1000);
    ...
    it('should prioritize recent messages', () => {
      const history = [
        { role: 'user', content: 'Old message about integration', timestamp: Date.now() - 3600000 },
        { role: 'assistant', content: 'Old response', timestamp: Date.now() - 3500000 },
        { role: 'user', content: 'Recent message about automation', timestamp: Date.now() - 60000 },
        { role: 'assistant', content: 'Recent response', timestamp: Date.now() - 30000 },
      ];
      
      const optimized = ContextWindowManager.optimizeConversationContext(history, 'query', 200);
      
      expect(optimized.length).toBeGreaterThan(0);
      expect(optimized[optimized.length - 1].content).toContain('Recent response');
    });
  });
});
...
describe('PromptOptimizationMetrics', () => {
  ...
  describe('recordOptimizationMetrics', () => {
    it('should record optimization metrics correctly', () => {
      ...
      const searchResults = [{ id: '1', content: 'result 1' }, { id: '2', content: 'result 2' }];
      ...
      const queries = [
        {
          ...
          optimizedQuery: { 
            metadata: { queryType: 'technical', complexity: 'basic', estimatedRelevance: 0.7 },
            expandedQueries: ['a', 'b'],
          },
          results: [1, 2],
          responseTime: 100,
        },
        {
          ...
          optimizedQuery: { 
            metadata: { queryType: 'configuration', complexity: 'intermediate', estimatedRelevance: 0.9 },
...
describe('Domain-specific prompts', () => {
  it('should have all required RoboRail domain configurations', () => {
    ...
    expectedDomains.forEach(domain => {
      expect(ROBORAIL_DOMAIN_PROMPTS[domain as keyof typeof ROBORAIL_DOMAIN_PROMPTS]).toBeDefined();
      
      const config = ROBORAIL_DOMAIN_PROMPTS[domain as keyof typeof ROBORAIL_DOMAIN_PROMPTS];
  ...
  it('should have comprehensive prompt templates', () => {
    ...
    expectedTypes.forEach(type => {
      expect(PROMPT_TEMPLATES[type as keyof typeof PROMPT_TEMPLATES]).toBeDefined();
...
describe('Integration tests', () => {
  it('should handle end-to-end optimization workflow', async () => {
    const query = 'How to troubleshoot RoboRail automation webhook authentication failures?';
    const context: QueryContext = {
      type: undefined, // Let it auto-classify
      domain: 'automation',
      conversationHistory: [
        { role: 'user', content: 'Setting up RoboRail webhooks', timestamp: Date.now() - 120000 },
        { role: 'assistant', content: 'Webhooks require proper authentication setup', timestamp: Date.now() - 60000 },
      ],
  ...
  it('should handle edge cases gracefully', async () => {
    ...
    const result = await PromptOptimizationEngine.optimizeQuery(basicQuery, minimalContext);
