...
export const AgentRequest = z.object({
  ...
  context: z.object({
    ...
    sources: z.array(z.enum(['openai', 'neon', 'memory'])).default(['openai']),
...
export const AgentRoutingDecision = z.object({
  ...
  suggestedSources: z.array(z.enum(['openai', 'neon', 'memory'])).default(['openai']),
...
export interface Agent {
  ...
  processRequestStream?(request: AgentRequest): AsyncGenerator<string, AgentResponse, unknown>;
  validateRequest(request: unknown): AgentRequest;
}

// Router interface
export interface AgentRouter {
  routeQuery(query: string, context?: AgentRequest['context']): Promise<AgentRoutingDecision>;
  classifyIntent(query: string): Promise<UserIntent>;
  analyzeComplexity(query: string): Promise<QueryComplexity>;
  selectAgent(intent: UserIntent, complexity: QueryComplexity, availableSources: z.infer<typeof VectorStoreType>[]): AgentType;
}
