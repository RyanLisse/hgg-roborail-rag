  ...
  annotations.forEach((annotation, index) => {
    if (annotation.type === 'file_citation' && annotation.file_citation) {
      ...
      if (source) {
        ...
        const citationNumber = citationMatches.find(match => 
          match.index && match.index >= annotation.start_index - 10 && 
          match.index <= annotation.end_index + 10
        );

        citations.push({
          id: `citation-${index}`,
          number: citationNumber ? Number.parseInt(citationNumber[1]) : index + 1,
 ...
 */
export function extractUniqueSourceFiles(citations: ParsedCitation[]): SourceFile[] {
